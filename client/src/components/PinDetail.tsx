  ...
  return (
    ...
    <section className="rounded-[32px] flex xl-flex-row flex-col m-auto bg-white max-w-[1500px]">
      <div className="flex justify-center items-center md:items-start flex-initial">
        <img src={pinDetails?.image && urlFor(pinDetails.image).url()} alt="user-post" className="rounded-t-3xl rounded-b-lg" />
      </div>
      <div className="w-full p-5 flex-1 xl:min-w-620">
        <div className="flex items-center justify-between">
          ...
          <a className="underline hover:text-gray-600 transition-all duration-200" href={`${pinDetails.destination}`} target="_blank">{pinDetails.destination}</a>
        </div>
        ...
        <Link className="flex gap-2 mt-5 items-center bg-white rounded-lg" to={`/user-profile/${pinDetails.postedBy?._id}`}>
        ...
        <div className="max-h-370 overflow-y-auto">
          {pinDetails.comments && pinDetails.comments.map((comment, i) => (
        ...
        <div className="flex flex-wrap mt-6 gap-3">
          <Link className="" to={`/user-profile/${pinDetails.postedBy?._id}`}>
